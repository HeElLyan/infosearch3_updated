...
for k in range(len(tokens)):
    ...
    for i in range(len(all_texts)):
        ...
        if count == 0:
            continue
        tokens_inverted_indexes[k] +=  ' ' + 'выкачка' + str(i + 1) + '.txt'
...
lemmas_inverted_indexes = keys.copy()

#i - file number, j - position of token in i file
for k in range(len(vals)):
    lemmas_inverted_indexes[k] += ':'
    for n in range(len(vals[k])):
      for i in range(len(all_texts)):
          ...
          lemmas_inverted_indexes[k] += ' ' + 'выкачка' + str(i + 1) + '.txt'
